...
function getCartTotalPrice($product_ids) {
	global $sql;

	$total_price_cart = 0;

	foreach ($product_ids as $product_id) {
		$stmt = $sql->prepare("SELECT price FROM product WHERE id = $product_id");
		$stmt->execute();
...
function getProduct($product_id) {
	...
	$stmt = $sql->prepare("SELECT product.id, product.price, product.stock, product.color, product_image.id
    						FROM product
    						INNER JOIN product_image
    						ON product.id=product_image.product
    						WHERE product.id = $product_id");
	$stmt->execute();
	$stmt->bind_result($id, $price, $stock, $color, $thumb);
	while ($stmt->fetch()) {
		$product = array('id'=> $id, 'price' => $price,
            'stock' => $stock,
            'color' => $color,
        	'thumb' => "binaries/products/img-$thumb-thumb.png");
	}
